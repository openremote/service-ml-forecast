...
def test_get_assets() -> None:
    assets: list[Asset] = openremote_client.get_assets("master")
    assert len(assets) > 0


def test_get_assets_invalid_realm() -> None:
    with pytest.raises(HTTPStatusError):
        openremote_client.get_assets("invalid_realm_name")


def test_get_asset_datapoint_period() -> None:
    datapoint_period: AssetDatapointPeriod = openremote_client.get_asset_datapoint_period("44ORIhkDVAlT97dYGUD9n5", "powerTotalConsumers")
    assert datapoint_period is not None


def test_get_asset_datapoint_period_invalid_asset_id() -> None:
    with pytest.raises(HTTPStatusError):
        openremote_client.get_asset_datapoint_period("invalid_asset_id", "powerTotalConsumers")
